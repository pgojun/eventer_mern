...
const AddEvent = () => {
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        dispatch(createEvent({ ...eventData, createdBy: user?.result?.name, creator: user?.result?._id }));
        navigate("/");
    }
            ...
            <div className="container max-w-sm mx-auto flex-1 flex flex-col items-center justify-center px-2">
                ...
                <form onSubmit={handleSubmit}>
                    <div className="bg-black/[.09] px-6 py-8 rounded shadow-md text-slate-400 w-full">
                        <h1 className="mb-8 text-3xl text-center">Add New Event</h1>
                        <input
                            type="text"
                        ...
                        <input
                            ...
                            className="bg-black/[.09] block border border-black w-full p-3 rounded mb-4"
                            name="eventTags"
                            placeholder="Event Tags (coma separated)"
                            onChange={(e) => setEventData({ ...eventData, eventTags: e.target.value.split(',') })}
                        />

                        <DateTimePicker className="py-4" name="dateAndTime" onChange={(val) => { setEventData({ ...eventData, dateAndTime: val }) }} value={eventData.dateAndTime} />
